...
import { removeProceeds } from '../../Store/Proceeds';

const Table = ({ proceeds, removeProceeds }) => {
  const { editActive, setEditActive, editID, setEditID } = useContext(MyContext);

  const deleteItem = id => removeProceeds(id);
...
export default connect(({ proceeds }) => ({ proceeds: proceeds.proceeds }), ({ removeProceeds }))(Table)